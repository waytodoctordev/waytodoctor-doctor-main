...
class DoctorDetailsApi {
  ...
  static Future<CreateCertificateModel?> createPictures({
    required File image,
  }) async {
    try {
      var imageStream = http.ByteStream(image.openRead());
      ...
      if (response.statusCode == 200) {
        ...
        var responseString = String.fromCharCodes(responseData);
        var jsonData = jsonDecode(responseString);
        CreateCertificateModel createPicturesModel =
  ...
  static Future<DeleteModel?> deletePayment({required String id}) async {
    try {
