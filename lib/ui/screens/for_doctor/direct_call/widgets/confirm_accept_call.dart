...
class ConfirmAcceptCallDialog extends StatelessWidget {
  final int patientId;

  const ConfirmAcceptCallDialog({
    Key? key,
          ...
          children: <Widget>[
            Text(
              MySharedPreferences.language == 'ar'
                  ? 'هل تريد الموافقة على طلب اتصال'
                  : "Are you sure",
                        ...
                        onPressed: () async {
                          ...
                          try {
                            await FirebaseFirestore.instance
                                ...
                                .update({
                              'callStatus': AppConstants.approved,
                            }).then((value) {
                              SendNotification().data(
                                  MySharedPreferences.fName,
                                  MySharedPreferences.language == 'ar'
                                      ? 'تم الموافقة على طلب الاتصال المباشر'
                                      : 'Call Request Accepted',
                                  patientId.toString());
                              Get.back();
                              AppConstants().showMsgToast(context,
                                  msg: AppConstants.updatedSuccessfully);
                            });
