...
import '../../api/registration/specialization_api.dart';
...
// single','engaged','married','devorced','widowed
  ...
  getFile({required BuildContext context, required bool isImage}) async {
    if (isImage) {
      print('hey nancy is iamge');
...
profileImage.value = image.path;
  ...
  Future<bool> editProfileImage({
    ...
    if (result != null) {
      ...
      isImageAdded.value = true;
      MySharedPreferences.profileImage = image;
      MySharedPreferences.userImage = '${ApiUrl.mainUrl}/${image.path}';

      profLicense = await CenterCtrl.find.professionalLicenseRequest(
          context: context,
          name: name,
          phone: phone,
          imageProfile: image,
          address: address,
          isEditCenterInfo: false);
  ...
  Future checkSpecialization({
    ...
    print(
        'checkSpecialization specializationCertificate$specializationCertificate');
