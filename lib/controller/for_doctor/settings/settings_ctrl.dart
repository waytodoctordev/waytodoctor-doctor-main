...
class SettingsCtrl extends GetxController {
  ...
  /// DONE . WORK HOURS ** REPLACED OF => WorkHoursForDoctorApi
  Future<void> fetchDoctorWorkHours() async {
    workHoursModel = await apiRequestService.makeRequest(
      method: AppConstants.getMethod,
      url:
          '${ApiUrl.mainUrl}${ApiUrl.workHoursForDoctor}/${MySharedPreferences.id}',
      fromJson: (json) => WorkHoursModel.fromJson(json),
    );
  ...
  /// DONE . DOCTOR'S ADDRESS DETAILS ** REPLACED OF => UpdateCommunicationForDoctorApi
  Future updateCommunication({
    ...
    if (doctorDetailsModel!.code == 200) {
      ...
      MySharedPreferences.lat = doctorDetailsModel!.data!.lat!;
      MySharedPreferences.lat = doctorDetailsModel!.data!.long!;
      AppConstants()
  ...
  /// DONE . CHANGE CLINIC PASSWORD ** REPLACED OF => UpdateClinicPasswordByDoctorApi
  ...
  // late Future<void> initializeWorkHoursForDoctor;
  ...
  @override
  void onInit() {
    pageController = PageController(initialPage: currentTabBarIndex);
